...
const GroupDetails = () => {
  ...
  // Hozirgi vaqtni olish — faqat bir marta hisoblanadi
  ...
  const [enrollTuitionFee, setEnrollTuitionFee] = useState('');
  const [enrollTeacherSalary, setEnrollTeacherSalary] = useState('');
...
const [attendanceDate, setAttendanceDate] = useState(
  ...
  // Add student mutation — now uses enrollments API
  ...
  // Remove student mutation — now uses enrollments API
  ...
  const handleAddStudent = useCallback((e) => {
      e.preventDefault();
      if (!selectedStudentId || !enrollTuitionFee) return;
      addStudentMutation.mutate({
          studentId: Number(selectedStudentId),
          groupId: Number(id),
          tuitionFee: parseFloat(enrollTuitionFee),
          teacherSalaryAmount: enrollTeacherSalary ? parseFloat(enrollTeacherSalary) : null
      });
  }, [selectedStudentId, enrollTuitionFee, enrollTeacherSalary, id, addStudentMutation]);

  const handleRemoveStudent = useCallback((student) => {
      if (!student.enrollmentId) {
          toast.error("Enrollment ID topilmadi");
          return;
      }
 ...
 const handleAttendanceSubmit = () => {
  ...
  const handleSendAbsentSms = useCallback((student) => {
  ...
  // Calculate statistics — useMemo bilan faqat group o'zgarganda qayta hisoblanadi
  // MUHIM: hooklar early return dan OLDIN bo'lishi shart!
  const { studentPayments, totalStudents, paidStudents, partialStudents, unpaidStudents, totalRevenue, expectedRevenue, remainingRevenue } = useMemo(() => {
    const sp = group?.studentPayments || [];
    const total = sp.length;
    const paid = sp.filter(s => s.paymentStatus === 'PAID').length;
    const partial = sp.filter(s => s.paymentStatus === 'PARTIAL').length;
    const unpaid = sp.filter(s => s.paymentStatus === 'UNPAID').length;
    
    const rev = sp.reduce((sum, s) => sum + (s.totalPaidInMonth || 0), 0);
    const expected = sp.reduce((sum, s) => sum + (Number(s.tuitionFee) || 0), 0);
    const remaining = expected - rev;

    return { studentPayments: sp, totalStudents: total, paidStudents: paid, partialStudents: partial, unpaidStudents: unpaid, totalRevenue: rev, expectedRevenue: expected, remainingRevenue: remaining };
  }, [group]);
              ...
              <div className="max-h-[60vh] overflow-y-auto">
                   ...
                   <div className="divide-y divide-gray-100">
                       {studentPayments.map(student => (
                           ...
                           >
                               <Link to={`/students/${student.studentId}`} className="text-base font-medium text-gray-900 hover:text-blue-600 transition-colors" onClick={(e) => e.stopPropagation()}>{student.studentName}</Link>
      ...
      >
          <form onSubmit={handleAddStudent} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">To'lov miqdori (so'm)</label>
                      <input
                          type="number"
                          required
                          min="1"
                  ...
                  <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">O'qituvchi ulushi</label>
                      <input
                          type="number"
                          min="0"
                          placeholder="100000"
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                          value={enrollTeacherSalary}
                          onChange={(e) => setEnrollTeacherSalary(e.target.value)}
                      />
                  </div>
              </div>
