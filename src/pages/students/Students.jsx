        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="date"
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
              value={formData.paymentDayOfMonth ? `${new Date().getFullYear()}-01-${String(formData.paymentDayOfMonth).padStart(2, '0')}` : ''}
              onChange={(e) => {
                  const date = new Date(e.target.value);
                  if (!isNaN(date.getTime())) {
                       setFormData({ ...formData, paymentDayOfMonth: date.getDate() });
                  } else {
                       setFormData({ ...formData, paymentDayOfMonth: '' });
                  }
              }}
            />
            <p className="text-xs text-gray-500 mt-1">Tanlangan sananing kuni ({formData.paymentDayOfMonth}-kun) har oy uchun takrorlanadi.</p>
          </div>
